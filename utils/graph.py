...
import torch
from voxel_hash_map.voxelize import (
    compute_graph_distances,
    compute_graph_distances_fast,
    compute_floyd_wharshall_distances,
)
import ipdb
...
class Graph:
    ...
    def build_graph_from_point_cloud(self, pnts):  # sparse_tensor):
        ...
        self.laplacian_matrix = np.full((self.num_vertices, self.num_vertices), np.inf)
        for i, p in enumerate(pnts):
            _, idx_neighbors, dist_neighbors = pcd_tree.search_hybrid_vector_3d(
                p, 0.05, self.knn
            )
            self.laplacian_matrix[i, idx_neighbors] = np.asarray(dist_neighbors)
        # compute_floyd_wharshall_distances(self.laplacian_matrix)
        # compute_graph_distances(self.laplacian_matrix)
        self.laplacian_matrix = compute_graph_distances_fast(self.laplacian_matrix)
